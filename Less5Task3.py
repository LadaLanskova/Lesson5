...
print("Здравствуйте! Вас приветствует игра Крестики - Нолики!\n")
...
print(f"Приятно познакомиться! Первым ходит {player1} и играет крестиками, {player2} играет ноликами! Поехали!\n")


move_dict = {1: f"Ход {player1}:", 0: f"ход {player2}:"}
simbol_dict = {1: " Х ", 0: " O "}
win_dict = {1: f"Крестики рулят! Победил {player1}!", 0: f"Нолики forever! Победил {player2}!"}
print()
...
while player_move < 10 and not win_flag:
    ...
    while not move_flag:
        ...
        if field[a][b] == "   ":
            ...
            print("Эта ячейка занята, выберите другую ячейку.\n")
    field_printing(field)
    print()
    if player_move > 4:    
